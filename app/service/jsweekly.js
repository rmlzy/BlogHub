 ...
 */
class JsWeeklyService extends Service {
  ...
  async _fetchPostList() {
    const { ctx } = this;
    const url = "https://javascriptweekly.com/issues";
    const list = [];
    const res = await ctx.curl(url, { type: "GET", dataType: "text" });
    ...
    $(".issues .issue")
      ...
      .map((item) => {
        list.push({
          url: "https://javascriptweekly.com/" + $(item).find("a").attr("href"),
          title: "JavaScript Weekly " + $(item).find("a").text(),
  ...
  async refresh(jumpExisted) {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      try {
        ...
        console.log(`❌ (${i + 1}/${list.length}) ${item.url}`);
        ctx.logger.error("Error while JsWeeklyService.refresh, stack: ", e);
      }
    }
  }
}

module.exports = JsWeeklyService;
